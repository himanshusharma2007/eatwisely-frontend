...
const ScanHistory = () => {
  ...
  useEffect(() => {
    const animateElements = () => {
      ...
      if (listRef.current) {
        ...
        setTimeout(() => {
          listRef.current.style.transition = 'all 0.6s cubic-bezier(0.4, 0, 0.2, 1)';
    ...
    const fetchScans = async () => {
      ...
      try {
        ...
        setScans((prevScans) => {
          ...
          const newScans = response.scans.filter((scan) => !existingIds.has(scan._id));
  ...
  const isLastMonth = (date) => {
    ...
    return scanDate.getMonth() === lastMonth.getMonth() && 
  ...
  const ScanList = ({ scans, title }) => (
    <div className="mb-8">
      ...
      {scans.length === 0 ? (
        <p className="text-slate-600">No scans found for this period.</p>
      ) : (
            ...
            >
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <div className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mt-2 ${getHealthScoreInfo(scan.analysis.healthScore).textColor} bg-white/80`}>
                    {scan.analysis.healthScore}/100 - {getHealthScoreInfo(scan.analysis.healthScore).label}
                  </div>
                </div>
              </div>
              <button
        ...
        <div ref={listRef}>
          ...
          {isLoading && scans.length === 0 ? (
            ...
            <div className="space-y-12">
              {yesterdayScans.length > 0 && <ScanList scans={yesterdayScans} title="Yesterday" />}
             { lastMonthScans.length > 0 && <ScanList scans={lastMonthScans} title="Last Month" />}
              ...
              {scans.length >= totalScans && scans.length > 0 && (
                <p className="text-center text-slate-600">No more scans to load.</p>
              )}
            </div>
          )}
        </div>
